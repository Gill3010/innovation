        ...
        <div className="relative w-full overflow-hidden rounded-2xl md:rounded-3xl shadow-2xl bg-white">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 min-h-[500px] md:min-h-[600px]">
            ...
            <div className="relative bg-linear-to-br from-[#0A1628] to-[#1a2942] p-6 sm:p-8 md:p-12 lg:p-16 flex flex-col justify-center">
              ...
              <div className="flex flex-wrap gap-3 sm:gap-4 landscape:gap-1.5 mb-6 landscape:mb-3 animate-fadeInUp animation-delay-600">
                {currentSlide.link && (
                  <a
                    href={currentSlide.link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-4 sm:px-8 py-2 sm:py-4 bg-linear-to-r from-[#00D4FF] to-[#0099FF] text-white font-bold text-xs sm:text-base landscape:text-[10px] rounded-full hover:shadow-2xl transition-all duration-300 transform hover:scale-105 landscape:shadow-lg landscape:border landscape:border-blue-500/50"
                    aria-label={`Learn more about ${currentSlide.title}`}
                  >
                    <span className="hidden landscape:block">Visit</span>
                    <span className="landscape:hidden">Visit Site</span>
                    <FaArrowRight className="w-2.5 h-2.5 landscape:w-2.5 sm:w-4 sm:h-4 group-hover:translate-x-1 transition-transform duration-300" />
                  </a>
                )}
                
                <button
                  onClick={() => openImageModal(currentSlide.image)}
                  className="inline-flex items-center gap-2 px-4 sm:px-8 py-2 sm:py-4 bg-white hover:bg-white/90 active:bg-white/80 text-slate-900 font-bold text-xs sm:text-base landscape:text-[10px] rounded-full border-2 border-white/80 hover:border-white active:border-white/70 transition-all duration-300 transform hover:scale-105 active:scale-95 shadow-lg landscape:bg-opacity-95"
                  aria-label="View full image"
                  style={{ touchAction: 'manipulation', WebkitTapHighlightColor: 'transparent' }}
                >
                  <svg className="w-2.5 h-2.5 landscape:w-2.5 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                  </svg>
                  <span className="hidden sm:inline">View Image</span>
                  <span className="sm:hidden landscape:hidden">View</span>
                </button>
              </div>
            </div>
          </div>
